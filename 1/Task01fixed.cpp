...
//FIXME Некорректный стиль названия функции;
//FIXME Названия аргументов неясно отражают их назначение;
...
void KnapSack(int W, vector<int>& wt, vector<int>& val) {
    ...
    int max_weight = 0; // Максимально возможный вес всех предметов
    for (int w : wt) max_weight += w; // Считаем общий вес всех предметов

    // Вектор dp, где dp[w] хранит минимальное налоговое бремя для веса w
    //FIXME Название переменной неясно отражает её назначение
    ...
    for (int i = 0; i < n; i++) { // Перебираем все предметы
        for (int w = max_weight; w >= wt[i]; w--) // Перебираем все возможные веса
        {
    ...
    for (int w = W; w <= max_weight; w++) // Перебираем все веса >= W
    {
